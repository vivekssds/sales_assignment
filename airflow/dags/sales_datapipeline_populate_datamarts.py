...
def sales_populate_datamarts():
    ...
    def populate_sales_by_customer():
        ...
        query = f"""Select sum(price) as total_sales,name as customer_name,id  from sales_raw.sales_raw_data_tbl
        group by name,id"""
        execute_query_and_write_to_table(
            query, "sales_by_customer_tbl", "sales_aggregate", mysql_connection_id
        )
    ...
    def populate_average_quality_per_product():
        ...
        mysql_connection_id = "mysql_conn"
        query = f"""Select AVG(quantity) as averagequantity,product_id  from sales_raw.sales_raw_data_tbl
            group by product_id """
        execute_query_and_write_to_table(
            query,
            "average_quality_per_product_tbl",
            "sales_aggregate",
            mysql_connection_id,
        )
    ...
    def populate_top5_products_by_sales():
        ...
        mysql_connection_id = "mysql_conn"
        query = f"""select product_id  as 'productid',sum(quantity) as 'nooforders' from sales_raw.sales_raw_data_tbl
            group by product_id order by 2 desc
            limit 5"""
        execute_query_and_write_to_table(
            query, "top5_products_by_sales_tbl", "sales_aggregate", mysql_connection_id
        )
    ...
    def analyse_monthly_sales():
        ...
        query = """SELECT 
            ...
            """
        execute_query_and_write_to_table(
            query, "monthly_sales_tbl", "sales_aggregate", mysql_connection_id
        )

    (
        populate_sales_by_customer()
        >> populate_average_quality_per_product()
        >> populate_top5_products_by_sales()
        >> analyse_monthly_sales()
    )
