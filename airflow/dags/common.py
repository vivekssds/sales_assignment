...
def fetch_weather_data(api_key, lat, lon):
    ...
    if response.status_code == 200:
        ...
        print(
            f"Failed to fetch weather data for lat={lat}, lon={lon}. Status code: {response.status_code}"
        )
        return None
...
def get_weather_info(users_sales_data):
    ...
    weather_data = fetch_weather_data(
        weather_api_key, users_sales_data["latitude"], users_sales_data["longitude"]
    )
    if weather_data:
